...
class MainViewModel: ViewModel() {
    private var productMemoryRepository: ProductMemoryRepository =
        ProductMemoryRepository(mutableListOf())
    ...
    fun updatePurchasedProduct(status: Boolean, position: Int){
        productMemoryRepository.updatePurchaseProduct(status, position)
    }
}